...
Like step 2b, collect large blast hits and reformat the table.
...
```bash
python3 scripts/03_extractSegments.py reads/short.fa.chopped.oriented.fa blastN/reference.vs.short.fa.chop.oriented.sorted.blastN -l *SIZE*
```
```bash
python3 scripts/03_extractSegments.py reads/long.fa.chopped.oriented.fa blastN/reference.vs.long.fa.chop.oriented.sorted.blastN -l *SIZE*
```
...
3. reads/short.fa.chopped.oriented.fa.**undetected.fa** an output fasta file that includes sequences occurring between blast hits in a single read, that are of a minimum size
   - Small gaps between blast hits are common and not necessarily indicative of a unique sequence (that is not a repeat segment)
